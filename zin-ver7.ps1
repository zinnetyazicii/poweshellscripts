...
$Button.Add_Click({
    ...
    if ($Username -ne "") {
        Try {
            ...
            ##if ($Groups) {
             ##   $GroupsListBox.Items.Add("👥 Grup Üyelikleri:")
            ##    $Groups | ForEach-Object { $GroupsListBox.Items.Add(" - $_") }
           ## }
...
$GroupsButton.Add_Click({   
    ...
    $GroupInfo = Get-ADGroup -Filter "Name -eq '$grupName'" -Properties GroupCategory, Mail
    $Groups = Get-ADGroupMember -Identity $grupName | Select-Object -ExpandProperty Name
    ...
    if ($grupName -ne "") {
        ...
        Catch {
            $GrupMember.Items.Clear()
            $GrupMember.Items.Add("Hata: Grup bulunamadı!")
        }
